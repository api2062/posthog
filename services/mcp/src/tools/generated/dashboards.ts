...
import {
    DashboardsAddInsightCreateBody,
...
const AddInsightToDashboardSchema = DashboardsAddInsightCreateParams.omit({ project_id: true }).extend(
    DashboardsAddInsightCreateBody.shape
)

const addInsightToDashboard = (): ToolBase<
    typeof AddInsightToDashboardSchema,
    Schemas.Dashboard & { _posthogUrl: string }
> => ({
    name: 'add-insight-to-dashboard',
    schema: AddInsightToDashboardSchema,
    handler: async (context: Context, params: z.infer<typeof AddInsightToDashboardSchema>) => {
        const projectId = await context.stateManager.getProjectId()
        const body: Record<string, unknown> = {}
        if (params.insight_id !== undefined) {
            body['insight_id'] = params.insight_id
        }
        const result = await context.api.request<Schemas.Dashboard>({
            method: 'POST',
            path: `/api/projects/${projectId}/dashboards/${params.id}/add_insight/`,
            body,
        })
        return {
            ...(result as any),
